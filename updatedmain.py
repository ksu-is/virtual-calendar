...
from tkinter import ttk, messagebox, simpledialog
    ...
    class Authentication:
        ...
        def register(self):
            while True:
                ...
                if username in self.users:
                    ...
                    password = simpledialog.askstring("Registration", "Enter a passwprd:")
        ...
        def login(self):
            while True:
                ...
                if self.validate_credentials(username,password):
                    ...
                    choice = messagebox.askquestion("Try Aggain?", "Do you want to try again?")
                    if choice.lower() != "yes":
                        return False
                    
        def validate_credentials(self,username,password):
            if username in awld.users and self.users[username] == password:
                return True
            else:
                return False
        ...
        def show_main_menu():
            ...
            main_menu_window.title("Virtual Calendar Menu")

            def display_calendar():
                year = int(year_entry.get())
                month = int(month_entry.get())
                planer.display_calendar(year, month)
            ...
            task_label.grid(row=4, column=0, padx=20, pady=10)
        ...
        root = tk.Tk()
        root.title("Virtual Calendar")

        authentication = Authentication()
        planner = virtualcalendar()
        ...
        welcome_label = tk.Label(root, text="Welcome to Virtual Calendar!", font=("Helvetica", 16))
